...
| 2026-07-29 | MQ.2 R2 recording-semantics contract | REGISTERED at clean `ac2cf03` before any R2 production/test/gate edit or binary run — the existing deterministic legal-zero 3,584-byte header remains frozen; a CRC-field-only corruption independently proves recomputation; all three 64-field CSV rows use nondegenerate steps `{7,11,19}` and independently retained live state/voltage bits; and the existing real-CUDA path receives a `true→false` mutation expected to fail exactly six density assertions. CSV adds 227 assertions without a case; the zero-CRC gate adds 14/1, taking Recorder 275/9→516/10. Old production must pass the CSV case at 257/1, reach 509/10 overall with only the seventh zero-CRC assertion red, and fail structure at the superseded zero-guard count 1 versus 0. The final three-lane, coverage, mutation, killed-design, and census bands are frozen in `.claude/reports/mq2-quality-backlog-preregistration-2026-07-23.md`. | REGISTERED; R2 remains open |
| — | NEXT | **MQ.2 R2 frozen old-production boundary:** add only the registered Recorder semantic/CRC oracles and zero-policy structural gate, prove unchanged production at CSV 257/1 green and Recorder 509 reached/10 with the correct-zero open alone red, commit that oracle-only boundary, then delete only the synchronous stored-zero guard. Run every registered mutation, three host lanes, real CUDA, exact coverage, and full suites; close R2 at Recorder 516/10 with `enqueuesnapshot-success-untested` REFUTED only if its six real-CUDA density assertions turn red. Then continue every remaining survivor batch to complete original 68+3 plus the append-only registry, MQ.3 hygiene → MQ.4 inventory → MQ.5 re-derivation → MQ.6 logic hunt → MQ.7 performance hunt → MQ.8 closeout, then M1.0 toward v6.0.0. |
