...
> and hot/no-throw contract is registered, and its old-production boundary is frozen at
> `b2c2f80`: behavior is green at 170/12 while structure is red exactly as registered.
...
- [ ] MQ.2 Quality-backlog owner box: triage EVERY remaining verified finding in
      ...
      **Mid-batch reconciliation completed 2026-07-24:** the four commits `c2940a2`,
      ...
      33 APPLIED / 34 pending / 9 deferrals. T2's 170/12 isothermal/no-throw contract is
      registered before test/source/gate edits or runs; old production passes the frozen
      170/12 oracle and fails only the future structural contract. Apply T2, then continue
      every remaining survivor batch until all 68 + 3 original
...
| — | NEXT | **MQ.2 T2 thermal implementation:** add only the paired `CompiledThermalGraph::assemble` `noexcept` specifications and the registered leading PackTopology owner/PLAN/hot/cold contract; do not touch the assembly body or frozen oracle. Require PackTopology 170/12 and aggregate structural 1/1, then run all seven registered mutation families, restore exact hashes, and pass the retained Debug/Release/CUDA focused plus 58/58 gates before dispositioning both T2 IDs. Proceed through every remaining survivor batch to complete original 68+3 plus post-baseline registry disposition, MQ.3 hygiene → MQ.4 inventory → MQ.5 re-derivation → MQ.6 logic hunt → MQ.7 performance hunt → MQ.8 closeout, then M1.0 toward v6.0.0. |
