...
| 2026-07-29 | MQ.2 R2 comparison-direction adversary | REGISTERED at clean implementation `612d841` after nine independent production/CSV mutation runs were red but before either newly identified self-comparison mutation ran — replacing either parsed accepted-step or parsed real-value operand with its expected operand preserves 516/10 and is predicted to leave the current loop-bound-only structural gate falsely green at 1/1. Both exact false-green demonstrations are required before adding compact pins for the two complete actual-versus-expected expressions; afterward each mutation must retain 516/10 but turn structure red. This is test-gate hardening only and changes no R2 production, count, or census band. | REGISTERED; adversarial run required |
| — | NEXT | **MQ.2 R2 one-line zero-CRC implementation:** commit the frozen oracle-only boundary, delete only `|| header.header_crc32 == 0`, and prove Recorder 516/10 plus structural 1/1. Then run every registered source/test mutation, three host lanes, real CUDA, exact coverage, and full suites; close R2 with `enqueuesnapshot-success-untested` REFUTED only if its six CUDA density assertions turn red. Continue directly through Q1, Q2, E1–E3, G1, and B1 until the survivor census is dry, then MQ.3→MQ.8 and M1.0 toward v6.0.0. |
