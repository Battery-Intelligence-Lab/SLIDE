...
> now preregistered; its failing-test-first boundary is next, then S2 rollback.
...
- [ ] MQ.2 Quality-backlog owner box: triage EVERY remaining verified finding in
      ...
      **Mid-batch reconciliation completed 2026-07-24:** the four commits `c2940a2`,
      ...
      final focused floors PackSolver 984/32 and PackStepper 333/13. Freeze and run the
      failing-test-first boundary before production moves; B1 also owns the newly observed
...
| — | NEXT | **MQ.2 S1.1 failing-test-first boundary:** implement and freeze the four move tests plus independent checkpoint-layout, analytic heterogeneous-thermal frozen-assembly, and non-adjacent prefix probes; run them against unchanged production and record both safe red move forms before implementing explicit moves. Then pass Debug/Release/host-C++ CUDA gates and controlled mutations, proceed to S2 source-step rollback, the remaining survivor batches to complete original 68+3 plus new-supplemental disposition, MQ.3 hygiene → MQ.4 inventory → MQ.5 re-derivation → MQ.6 logic hunt → MQ.7 performance hunt → MQ.8 closeout, then M1.0 toward v6.0.0. |
