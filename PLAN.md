...
> signature/comment/placement/numerical mutations turn red. C1's 886/10 exact
> grammar/diagnostic and future structural gates are registered before any C1 edit or run.
...
- [ ] MQ.2 Quality-backlog owner box: triage EVERY remaining verified finding in
      ...
      **Mid-batch reconciliation completed 2026-07-24:** the four commits `c2940a2`,
      ...
      edit or binary run. Freeze the old-production boundary, then apply C1 and
      continue every remaining survivor batch until all 68 + 3 original
...
| — | NEXT | **MQ.2 C1 frozen old-production boundary:** add only the registered NetlistCsv unit oracles and future structural gate. Require unchanged production to pass exactly 886/10 while the aggregate structural test is red first at `msg_too_large` 0/1; record hashes and freeze that oracle-only commit before touching `NetlistCsv.cpp`, `.hpp`, or `CHANGELOG.md`. Then implement C1 literally, mutation-test every owner, and pass the retained Debug/Release/CUDA focused plus 58/58 gates before dispositioning the three IDs. Proceed through every remaining survivor batch to complete original 68+3 plus post-baseline registry disposition, MQ.3 hygiene → MQ.4 inventory → MQ.5 re-derivation → MQ.6 logic hunt → MQ.7 performance hunt → MQ.8 closeout, then M1.0 toward v6.0.0. |
