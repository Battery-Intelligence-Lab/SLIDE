...
- [ ] MQ.2 Quality-backlog owner box: triage EVERY remaining verified finding in
      ...
      **Mid-batch reconciliation completed 2026-07-24:** the four commits `c2940a2`,
      ...
      P1 then APPLIED `cell-current-reconstruction-x4`, `branch-drop-and-kcl-current`, and
      `dead-usings-and-misplaced-comment` at `8630207`: all five affine operations have one checked
      owner, the exact 949/30 traces and aggregate structural gate pass in the same three retained
      configurations, and five independent algebra/gate mutations turn red. Evidence:
      `.claude/reports/mq2-quality-backlog-validation-2026-07-23.md`. P2 then APPLIED
      ...
      30 APPLIED / 37 pending / 9 deferrals across 76 stable IDs. T1 PASSED locally at
      `ad6d785` plus gate hardening `30464e4` and formatting `479f55f`: one sorted `BatchSlot`
      map owns batch/lane/thermal metadata, one guarded `BranchGraph` owner derives adjacency
      and canonical sparsity for generated/imported paths, and one BFS owns connectivity.
      PackTopology 164/11 and all companion focused gates pass in Debug, fast-math
      Release/IPO-off, and host-C++ CUDA; all three final trees pass 58/58 and no-op rebuilds.
      Nine derivation/connectivity/sparsity/order/lane/thermal/rollback mutations turn red.
      Original census: 29 APPLIED / 34 pending / 8 deferrals; combined 76-ID registry:
      33 APPLIED / 34 pending / 9 deferrals. T2 PASSED locally at `72368f2` plus
      `0f564ce`/`db1b067`: paired `noexcept` specifications and a prefix-sliced
      owner/PLAN/hot/cold contract leave the frozen exact-zero oracle green at
...
| 2026-07-29 | MQ.2 T1 topology executable contract | REGISTERED before any T1 test/source edit or binary run — one independent eight-assertion compiled-topology fixture pins complete cell, batch/lane, branch, sparsity, ladder, and empty-thermal metadata; one seven-assertion imported-orientation fixture requires all three ladder vectors to roll back; and one direct hostile-endpoint assertion prevents unsafe shared graph construction. PackTopology is frozen at +16/+2, exactly 164/11. The structural gate requires one `BranchGraph`/`buildBranchGraph` owner with two callers, one connectivity owner with two callers, endpoint/cell validation before graph construction, and one sorted `BatchSlot` map replacing all three parallel maps. Seven mutation families and the retained three-configuration focused plus 58/58 gates are registered in `.claude/reports/mq2-quality-backlog-preregistration-2026-07-23.md`. | REGISTERED; T1 remains open |
| 2026-07-29 | MQ.2 T1 frozen old-production boundary | PASSED behavior / RED structure exactly as registered at oracle-only `17538de`: production `PackTopology.cpp` remains at SHA-256 `C06F37…B626`; Debug PackTopology passes 164/11 with the complete exact topology, hostile-endpoint, and all-three-vector rollback fixtures, while the aggregate structural test passes every prior included suite and then fails solely at `BatchSlot` count 0 versus 1. The first build stopped before link/run on Catch2 compound-predicate syntax; parenthesizing five unchanged expressions restored the registered count. No Release/CUDA/full-suite or production claim. Artifact: `.claude/reports/mq2-quality-backlog-validation-2026-07-23.md`. | FROZEN; implementation required |
| 2026-07-29 | MQ.2 T1 topology ownership | PASSED locally at `ad6d785` plus gate hardening `30464e4` and formatting `479f55f` — one sorted `BatchSlot` map replaces three parallel maps; one guarded `BranchGraph` owner plus one BFS serves generated and imported metadata while retaining the P9-B18 size bound and complete hostile endpoint/cell validation before allocation/indexing. The frozen complete topology and rollback fixtures remain exact at PackTopology 164/11. Debug, fast-math Release/IPO-off, and host-C++/CUDA-tree focused gates each also pass PackSolver 994/33, PackStepper 333/13, allocation 14/2, and structural 1/1; all three final trees pass 58/58, including real CUDA, then no-op rebuild. Nine duplicate/connectivity/sparsity/order/lane/thermal/three-vector rollback mutations turn red; an initially wrong-site inverse patch was caught by SHA-256 before build/run. Final source/test/structure hashes are recorded. The three T1 IDs are APPLIED: original census 29 APPLIED / 34 pending / 8 deferrals; combined census 33 APPLIED / 34 pending / 9 deferrals across 76 IDs. Artifact: `.claude/reports/mq2-quality-backlog-validation-2026-07-23.md`. | PASSED locally; MQ.2 remains open |
| 2026-07-29 | MQ.2 T2 thermal-topology executable contract | REGISTERED before any T2 production, unit-test, or structural-gate edit and before any T2 binary run — one five-edge triangle-plus-two-boundaries fixture poisons every output/scratch surface, proves degree-three incidence offsets `{0,3,5,8,9,10}`, and requires exact zero cell heat, boundary heat, and edge flux at a uniform finite 300 K. It adds exactly 6 assertions / 1 case, taking PackTopology 164/11→170/12. The future structural gate requires exact `noexcept` declaration/definition tokens plus one substantive owner/PLAN §3.4 D-19/D-21/hot/cold leading contract and forbids the stale all-cold brief. The oracle-only boundary must be old-production behavior-green 170/12 and structure-red first at declaration count 0 versus 1. Seven signature, comment, arithmetic, and independent publication mutation families plus the retained three-lane focused/full gates are registered in `.claude/reports/mq2-quality-backlog-preregistration-2026-07-23.md`. | REGISTERED; T2 remains open |
| 2026-07-29 | MQ.2 T2 frozen old-production boundary | PASSED behavior / RED structure exactly as registered at oracle-only `b2c2f80`: unchanged `PackTopology.cpp`/`.hpp` hashes pass the poisoned five-edge isothermal fixture and complete PackTopology 170/12, while the aggregate structural test passes all prior included suites and fails first at exact `noexcept` declaration count 0 versus 1. The fixture also pins offsets `{0,3,5,8,9,10}` and exact-zero cell heat, boundary heat, and published edge flux. No Release/CUDA/full-suite or production claim. Artifact: `.claude/reports/mq2-quality-backlog-validation-2026-07-23.md`. | FROZEN; implementation required |
...
| 2026-07-29 | MQ.2 R2 recording-semantics contract | REGISTERED at clean `ac2cf03` before any R2 production/test/gate edit or binary run — the existing deterministic legal-zero 3,584-byte header remains frozen; a CRC-field-only corruption independently proves recomputation; all three 64-field CSV rows use nondegenerate steps `{7,11,19}` and independently retained live state/voltage bits; and the existing real-CUDA path receives a `true→false` mutation expected to fail exactly six density assertions. CSV adds 227 assertions without a case; the zero-CRC gate adds 14/1, taking Recorder 275/9→516/10. The first old-source run corrected only the pre-existing CSV-case subtotal 30→31, so its exact total is 258/1; the registered whole-target 509/10 and final 516/10 bands are unchanged. Old production reaches only the seventh zero-CRC assertion red and fails structure at the superseded zero-guard count 1 versus 0. The final three-lane, coverage, mutation, killed-design, and census bands are frozen in `.claude/reports/mq2-quality-backlog-preregistration-2026-07-23.md`. | REGISTERED; R2 remains open |
| 2026-07-29 | MQ.2 R2 frozen old-production boundary | PASSED semantics / RED behavior and structure exactly as registered at oracle WIP after `30afab7` — unchanged production passes every one of the three independently parsed 64-field CSV rows at 258/1; the full Recorder target reaches 509/10 with 508 passing assertions and fails only the seventh new assertion because the legal zero-CRC file is rejected; Debug companion gates remain 471/12, 33/3, and 60/4. The aggregate structural test passes every preceding included suite and fails only at stored-zero guard count 1 versus the R2 policy 0. All registered production hashes remain exact; oracle/gate hashes and the transparent 257→258 subtotal correction are recorded in `.claude/reports/mq2-quality-backlog-validation-2026-07-23.md`. | FROZEN; implementation required |
| 2026-07-29 | MQ.2 R2 comparison-direction adversary | REGISTERED at clean implementation `612d841` after ten independent production/CSV mutation runs were red but before either newly identified self-comparison mutation ran — replacing either parsed accepted-step or parsed real-value operand with its expected operand preserves 516/10 and is predicted to leave the current loop-bound-only structural gate falsely green at 1/1. Both exact false-green demonstrations are required before adding compact pins for the two complete actual-versus-expected expressions; afterward each mutation must retain 516/10 but turn structure red. The registration commit said nine; the validation record transparently corrects that transcription count to the already executed three CRC plus seven CSV runs. This is test-gate hardening only and changes no R2 production, count, or census band. | REGISTERED; adversarial run required |
| 2026-07-29 | MQ.2 R2 Debug mutations and comparison hardening | PASSED at implementation `612d841` with test hash `098ba33` restored and structural hardening WIP `dcb34f0` — all ten CRC/CSV mutations turn the intended behavior and/or structure red; their isolated R2 failures are 1 stored-zero, 1 computed-zero, 1 corrupt-CRC recomputation, 3 step, 2 time, 3 current, 144 state-stride, 48 state-lane, 3 voltage, and 42 precision assertions. Both preregistered actual→expected self-comparisons first reproduced the exact 516/10 plus structural 1/1 false green. After two compact direction pins, each still passes 516/10 but structure fails at its named pin. Weakening the row loop remains unit-green at only 452/10 and structural-red. Restored Debug is 516/10 plus aggregate structural 1/1. Artifact: `.claude/reports/mq2-quality-backlog-validation-2026-07-23.md`. | PASSED; CUDA mutation and final acceptance remain |
| — | NEXT | **MQ.2 R2 CUDA discriminator and acceptance:** commit comparison-direction hardening, run the real-CUDA `enqueueSnapshot true→false` mutation for exactly 433,665 pass / 6 fail and restore 433,671/4. Then run focused and full Debug, fast-math Release/IPO-off, and host-C++ CUDA gates, no-op rebuilds, fresh WSL coverage, formatting, and final adversarial review; close R2 and its census before continuing directly through Q1, Q2, E1–E3, G1, and B1 until MQ.2 is dry, then MQ.3→MQ.8 and M1.0 toward v6.0.0. |
