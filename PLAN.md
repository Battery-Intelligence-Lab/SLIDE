...
> and hot/no-throw contract is registered; its old-production oracle boundary is next.
...
- [ ] MQ.2 Quality-backlog owner box: triage EVERY remaining verified finding in
      ...
      **Mid-batch reconciliation completed 2026-07-24:** the four commits `c2940a2`,
      ...
      registered before test/source/gate edits or runs. Freeze its old-production boundary,
      then apply T2 and continue every remaining survivor batch until all 68 + 3 original
...
| — | NEXT | **MQ.2 T2 frozen old-production boundary:** add only the registered +6/+1 isothermal oracle and future structural checks. Require unchanged production to pass PackTopology 170/12 while aggregate structure fails at the preregistered missing declaration token; record exact hashes before implementing. Then add only the paired `noexcept` specifications and leading PackTopology contract comment, run all seven registered mutation families, restore exact hashes, and pass the retained Debug/Release/CUDA focused plus 58/58 gates before dispositioning both T2 IDs. Proceed through every remaining survivor batch to complete original 68+3 plus post-baseline registry disposition, MQ.3 hygiene → MQ.4 inventory → MQ.5 re-derivation → MQ.6 logic hunt → MQ.7 performance hunt → MQ.8 closeout, then M1.0 toward v6.0.0. |
