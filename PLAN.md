...
and ODE-row roles (§3.12/D-23), plus the first shared observable kernel (SPM surface concentration: exact legacy
parity and 1.276e-15 analytic round-trip). NOT STARTED: the rest of the Deliver list — `Domain`/`ElectrodeParams`
(§3.3 Layer-1 types), remaining observables, `ThermalLumped`, ageing kernels, registry + factory, `Simulation`
façade — and gates P1-G1/G2/G3(b)/G4.
...
**Ordering:** (1) observable layer + `BatchView`/`StepCtx` — **IN PROGRESS: rebindable views, row roles, and surface
concentration implemented 2026-07-10; centre concentration/kinetics/voltage remain** → (2) P1-G3 Chebyshev
...
| 2026-07-10 | Phase-1 rebindable view + first observable | DONE — `BatchView`/`RhsViews` rebind arbitrary integrator trial vectors without copies; mandatory ydot-zero stage and ODE/algebraic/cumulative/input row roles implemented (D-23). Surface concentration is now one scalar-generic free function shared by future RHS/Recorder paths: legacy parity rel=0 both electrodes; heterogeneous analytic round-trip max rel=1.276e-15. Hardened arena moves/slice bounds and diffusion lane bound; full Debug suite 15/15 green. |
| — | Phases 1–8 | Phase 1 IN PROGRESS (DONE: StateArena/BatchBuilder + tests, P1-G0 pilot + Release re-confirm, production `SpectralDiffusion<NCH>`, rebindable BatchView/StepCtx + row roles, first surface-concentration observable. **NEXT: finish observable outputs and P1-G3(b) Carslaw–Jaeger transient/centre-node oracle**; then ThermalLumped → ageing (promote `_prev`/accumulators to arena rows) → registry/factory + façade + EulerLegacy stepper → P1-G1/G2/G4 → PAY-1); Phases 2–8 not started; D-21 (pack thermal — adopt LAMMPS-style static adjacency pair list with fixed-order accumulation, per §3.8 determinism rule) still due before Phase 2 |
