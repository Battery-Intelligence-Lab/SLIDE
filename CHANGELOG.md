...
* **P8-G4 persistent parallel runtime** (`src/core/ThreadPool.*`): a dependency-free `std::thread` pool creates workers once, assigns every indexed task exactly once through an atomic work cursor, waits without busy-spinning, propagates the first `Status`/exception only after deterministic task completion, and shuts down cleanly. `fixedOrderSum` makes reduction order independent of scheduling/worker count. `slide::test::parallelisation()` reports logical cores, backend, workers, serial/parallel timings, checksum, and informational speedup. Exact-once/failure/reuse, worker-count bit-repeatability, Debug, and Release gates pass.
...
* **Phase-4 Mode C relaxation scale path** (`src/core/PackSolver.*`): compiled index-1 netlists can use preallocated weighted-Jacobi waveform relaxation with an explicit Baumgarte gain, per-iteration constraint-drift/bound diagnostics, warm nodal/current state, and zero sparse factorizations. Compile metadata refuses the mode when the index-1 criterion is absent. P4-G1's heterogeneous 16p currents agree with Mode A within `0.1%`; P4-G2 verifies measured terminal-KCL drift stays below the registered `(1-α)^k` bound. P4-G3/PAY-3 builds and accepts two consecutive 100,000-cell SPM pack advances with a 24 MB arena state, below 1 GB, and exactly zero allocations on the measured advance.
