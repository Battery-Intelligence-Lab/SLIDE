...
double CyclerOld::getCapacity(bool blockDegradation)
{
	/*
	...
	if constexpr (printBool::printCyclerFunctions)
		std::cout << "CyclerOld::getCapacity is starting\n";

	// *********************************************************** 1 variables & settings ***********************************************************************

	// variables

	const auto s = c.getStates();			// initial battery state
	const auto Iold = c.getI();				// initial battery current
	...
	try
	{
		...
		if constexpr (printBool::printCyclerHighLevel) // print high level flow of the simulation if desired
			std::cout << "CyclerOld::getCapacity going to fully charge the cell.\n";
		...
		if constexpr (printBool::printCyclerHighLevel) // print high level flow of the simulation if desired
			std::cout << "CyclerOld::getCapacity going to CC discharge the cell.\n";
		...
		if constexpr (printBool::printCyclerHighLevel)					  // print high level flow of the simulation if desired
			std::cout << "CyclerOld::getCapacity going to CV discharge the cell.\n";
		CV_I(c.getVmin(), dt, blockDegradation, Icut, &CVcap, &Ven, &timei); // do a CV discharge until the current is below the cut-off current
	}
	catch (int e)
	{
		// std::cout << "Throw test: " << 42 << '\n';
		if constexpr (printBool::printCrit)
			std::cout << "Error in a subfunction of CyclerOld::getCapacity " << e << ". Throwing it on.\n";
		throw e;
	}
	...
	if constexpr (printBool::printCyclerFunctions)
		std::cout << "CyclerOld::getCapacity is terminating with a capacity of " << CCcap + CVcap << "Ah.\n";
