...
// push a write such that if cells still have cycling data, this is written
	...
	void Procedure::checkUp_writeStats(std::vector<Cell *> &cells, std::ofstream &file, int separator)
	{
		...
		if (file.is_open())
		{
			...
			for (int i = 0; i < settings::DATASTORE_NHIST; i++)
			{ // loop for each bin in the stats of I (rows)
				file << ID_histI << ',';
				for (auto &_ : cells)
				{ // loop for each cell (columns)
			...
			for (int i = 0; i < settings::DATASTORE_NHIST; i++)
			{ // loop for each bin in the stats of V (rows)
				file << ID_histV << ',';
				for (auto &_ : cells)
				{ // loop for each cell (columns)
					//	cell->getHist(&histI, &histV, &histT, nout);
			...
			for (int i = 0; i < settings::DATASTORE_NHIST; i++)
			{ // loop for each bin in the stats of T (rows)
				file << ID_histT << ',';
				for (auto &_ : cells)
				{ // loop for each cell (columns)
