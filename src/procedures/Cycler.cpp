...
namespace slide
{
	...
	Status Cycler::setCurrent(double I, double vlim)
	{
		...
		if (I < 0)
		{ // charging -> exceeded if Vini < vlim  & Vnew > vlim
	...
	Status Cycler::CV(double Vset, double Ilim, double tlim, double dt, int ndt_data, double &Ah, double &Wh, double &ttot)
	{
		...
		if (std::abs(Ii) < 1e-3)
		{ // if no current yet, set a small one
		...
		else
		{
			...
			while (succ > Status::Success)
			{ // we could not set the current, try starting with a lower current
				Ii = Ii / 2.0;
		...
		for (size_t i = 0; i < nt; i++)
		{
			...
			while (!reach)
			{
				...
				else
				{
					...
					if (std::abs(Ii) < 1e-6)
					{ // avoid eternal loop with very small currents by setting 0A, which should always work
						Ii = 0;
