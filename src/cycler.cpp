...
double Cycler::getCapacity(bool blockDegradation)
{
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::getCapacity is starting\n";

	// *********************************************************** 1 variables & settings ***********************************************************************

	// variables
	slide::State s;							// initial battery state
	...
	try
	{
		...
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel) // print high level flow of the simulation if desired
			std::cout << "Cycler::getCapacity going to fully charge the cell.\n";
		...
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel) // print high level flow of the simulation if desired
			std::cout << "Cycler::getCapacity going to CC discharge the cell.\n";
		...
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel) // print high level flow of the simulation if desired
			std::cout << "Cycler::getCapacity going to CV discharge the cell.\n";
	...
	catch (int e)
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in a subfunction of Cycler::getCapacity " << e << ". Throwing it on.\n";
		throw e;
	}
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::getCapacity is terminating with a capacity of " << CCcap + CVcap << "Ah.\n";
...
void Cycler::getOCV(slide::fixed_data<double> &Ah_fixed, std::vector<double> &OCVp, std::vector<double> &OCVn)
{
	/*
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::getOCV is starting\n";
	...
	try
	{
		...
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::getOCV is fully charging the cell to the maximum cell voltage.\n";
		...
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::getOCV is fully charging the cathode.\n";
		...
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::getOCV is fully discharging the cathode.\n";
		...
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::getOCV is fully charging the anode.\n";
		...
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::getOCV is fully discharging the anode.\n";
	...
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in a subfunction of Cycler::getOCV when half-cycling the cells" << e << ". Throwing it on.\n";
		throw e;
	}

	// *********************************************************** 3 make a common x-axis ***********************************************************************
	if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
		std::cout << "Cycler::getOCV is making the x-axis\n";
	...
	slide::fixed_data<double> Ahpi_vec_fixed(ah_neg_p, (dt / 3600.0 * Idis), np);

	// make the x-axis for the anode, exactly the same as for the cathode
	slide::fixed_data<double> Ahni_vec_fixed(ah_neg_n, (dt / 3600.0 * Idis), nn);
	...
	if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
		std::cout << "Cycler::getOCV is aligning the half-cell curves\n";
	...
	Ah_fixed = slide::fixed_data<double>(Ahmin, dAh_fixed, n_elem);
	...
	for (int i = 0; i < Ah_fixed.size(); ++i)
	{
		...
		const double ocvp = linInt(settings::verbose >= printLevel::printCrit, bound, Ahpi_vec_fixed, OCVpi_vec, np, Ah_fixed[i], is_Ahpi_fixed); // interpolate the negative half cell curve at the x-point we're at
		const double ocvn = linInt(settings::verbose >= printLevel::printCrit, bound, Ahni_vec_fixed, OCVni_vec, nn, Ah_fixed[i], is_Ahni_fixed); // interpolate the positive half-cell curve at the x-point we're at
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::getOCV terminating.\n";
}
...
double Cycler::checkUp_batteryStates(bool blockDegradation, bool checkCap, int cumCycle, double cumTime, double cumAh, double cumWh)
{
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::checkUp_batteryStates is starting.\n";

	// variables
	slide::State si; // initial battery state
	double Ii;		 // initial battery current [A]

	// Get the capacity and the states
	c.getStates(si, &Ii);								   // State-object with the cell's state
	const slide::states_type &states = si.getStates_arr(); // array with the battery states
	double cap;											   // the remaining capacity [Ah]
	try
	{
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::checkUp_batteryStates is getting the capacity.\n";
		if (checkCap)
	...
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in Cycler::checkUp_batteryStates when measuring the cell capacity: " << e << ". Throwing it on.\n";
		throw e;
	}

	// Write the results to the csv file
	if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
		std::cout << "Cycler::checkUp_batteryStates is writing the capacity to the csv file.\n";
	...
	if (!output.is_open())
	{
		...
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cerr << "ERROR in Cycler::checkUp_batteryStates. File " << fol + "DegradationData_batteryState.csv"
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::checkUp_batteryStates terminating.\n";
...
void Cycler::checkUp_OCVcurves(bool blockDegradation, double ocvpini, double ocvnini)
{
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::checkUp_OCVcurves is starting.\n";
	...
	{
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::checkUp_OCVcurves is getting the OCV curves.\n";
		getOCV(ocvAh, ocvp, ocvn); // measure the OCV curves
	}
	catch (int e)
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in Cycler::checkUp_OCVcurves when measuring the OCV curves: " << e << ". Throwing the error on.\n";
		throw e;
	}

	// Write the curves to the csv file
	if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
		std::cout << "Cycler::checkUp_OCVcurves is writing the OCV curves to a csv file.\n";
	...
	if (!output.is_open())
	{
		// we couldn't open the file
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cerr << "ERROR on Cycler::checkUp_OCVcurves. File " << fol + "DegradationData_OCV.csv"
					  << " could not be opened. Throwing an error.\n";
		throw 1001;
	}
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::checkUp_OCVcurves terminating\n";
}
...
void Cycler::checkUp_CCCV(bool blockDegradation, int nCycles, double Crates[], double Ccut_cha, double Ccut_dis, bool includeCycleData)
{
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::checkUp_CCCV starting\n";
	...
	{
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::checkUp_CCCV is charging the cell before doing the CCCV cycles.\n";
		CC_V_CV_I(1.0, c.getVmax(), Ccut_cha, dt, blockDegradation, &ahi, &whi, &timei); // 1C charge to the specified cutoff Crate
	}
	catch (int e)
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in Cycler::checkup_CCCV when initially charging the cell: " << e << ". Throwing it on.\n";
		throw e;
	}
	...
	{
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::checkUp_CCCV is flushing the previously stored cycling data.\n";
		writeCyclingData();
	}
	catch (int e)
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in Cycler::checkup_CCCV when writing the original cycling data: " << e << ".\n";
		throw e;
	}
	...
	for (int i = 0; i < nCycles; i++)
	{
		...
		{
			if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
				std::cout << "Cycler::checkUp_CCCV is simulating a CCCV cycle at Crate = " << Crates[i] << ".\n";
		...
		{
			if constexpr (settings::verbose >= printLevel::printCrit)
				std::cout << "Error in Cycler::checkup_CCCV when following CCCV cycle at Crate: " << Crates[i]
	...
	{
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::checkUp_CCCV is writing the cycling data from the CCCV cycles.\n";
		writeCyclingData(nameCCCV, !includeCycleData); // if we don't want to include the cycling data, clear the buffer
	}
	catch (int e)
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in Cycler::checkup_CCCV when writing the cycling data from the CCCV cycles: " << e << ". Throwing it on.\n";
		throw e;
	}
...
void Cycler::checkUp_pulse(bool blockDegradation, std::string profileName, int profileLength, bool includeCycleData)
{
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::checkUp_pulse starting.\n";
	...
	if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
		std::cout << "Cycler::checkUp_pulse is checking if the current profile is a discharge.\n";

	const double aht = std::inner_product(I.begin(), I.end(), T.begin(), 0.0); // total charge throughput of the profile [As]

	if (aht <= 0)
	{ // if the total charge throughput is negative, the profile is a net charge
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cerr << "ERROR in Cycler::checkUp_pulse: the pulse profile is not a net discharge. "
	...
	{
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::checkUp_pulse is charging the cell.\n";
	...
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in Cycler::checkup_pulse when initially charging the cell: " << e << ".\n";
	}
	...
	{
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::checkUp_pulse is flushing out the previously stored cycling data.\n";
		writeCyclingData();
	}
	catch (int e)
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in Cycler::checkup_pulse when writing the original cycling data: " << e << "\n.\n";
		throw e;
	}
	...
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in Cycler::checkup_pulse when following the pulses: " << e << ".\n";
	}
	...
	{
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::checkUp_pulse is writing the cycling data from the pulse profile.\n";
		writeCyclingData(namePulse, !includeCycleData); // if we don't want to include the cycling data, clear the buffer
	}
	catch (int e)
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in Cycler::checkup_pulse when writing the cycling data from the pulse discharge: " << e << ". Throwing it on.\n";
		throw e;
	}
...
double Cycler::checkUp(struct checkUpProcedure proc, int cumCycle, double cumTime, double cumAh, double cumWh)
{
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::checkUp starting\n";

	// variables
	slide::State sini;							// initial cell state
	...
	c.getVoltage(settings::verbose >= printLevel::printCrit, &v, &ocvpini, &ocvnini, &etap, &etan, &rdrop, &tcellini); // initial cell voltage and temperature
	...
	if (!proc.includeCycleData)
	{
		...
		{
			if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
				std::cout << "Cycler::checkUp is flushing the previously stored cycling data.\n";
		...
		{
			if constexpr (settings::verbose >= printLevel::printCrit)
				std::cout << "Error in Cycler::checkUp when writing the previously stored cycling data: " << e << ".\n";
			throw e;
		}
	}
	...
	{
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::checkUp is bringing the cell to just below its maximum voltage.\n";
		CC_V_CV_I(1.0, c.getVmax() - 0.2, 0.05, dt, proc.blockDegradation, &ahi, &whi, &timei); // 1C current, 0.05C cutoff current
	}
	catch (int e)
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in Cycler::checkUp when setting the cell voltage to Vmax-0.2V: " << e << ". Throwing it on.\n";
		throw e;
	}
	...
	if (proc.capCheck)
	{
		...
		{
			if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
				std::cout << "Cycler::checkUp is starting a capacity check.\n";
			cap = checkUp_batteryStates(proc.blockDegradation, true, cumCycle, cumTime, cumAh, cumWh);
		}
		catch (int e)
		{
			if constexpr (settings::verbose >= printLevel::printCrit)
				std::cout << "Error in Cycler::checkUp when measuring the cell capacity: " << e << ". Skip the capacity measurement.\n";
		}
	}
	...
	if (proc.OCVCheck)
	{
		...
		{
			if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
				std::cout << "Cycler::checkUp is starting an OCV check.\n";
			checkUp_OCVcurves(proc.blockDegradation, ocvpini, ocvnini);
		}
		catch (int e)
		{
			if constexpr (settings::verbose >= printLevel::printCrit)
				std::cout << "Error in Cycler::checkUp when measuring the OCV curves: " << e << ". Skip the OCV measurements.\n";
			throw e;
		}
	}
	...
	if (proc.CCCVCheck)
	{
		...
		{
			if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
				std::cout << "Cycler::checkUp is starting a CCCV cycle check.\n";
			checkUp_CCCV(proc.blockDegradation, proc.nCycles, proc.Crates, proc.Ccut_cha, proc.Ccut_dis, proc.includeCycleData);
		}
		catch (int e)
		{
			if constexpr (settings::verbose >= printLevel::printCrit)
				std::cout << "Error in Cycler::checkUp when measuring the CCCV cycles: " << e << ". Skip the CCCV measurement.\n";
		}
	}
	...
	if (proc.pulseCheck)
	{
		...
		{
			if constexpr (settings::verbose >= printLevel::printCrit)
				std::cout << "Error in Cycler::checkUp when measuring the pulse test: " << e << ". Skip the CCCV measurement.\n";
		}
	}
	...
	if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
		std::cout << "Cycler::checkUp is restoring the initial battery state.\n";
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::checkUp terminating with capacity " << cap << "Ah.\n";
						 ...
						 double Cdis, bool CVdis, double Ccutdis, double Ti, int nrCycles, int nrCap, struct checkUpProcedure proc)
{
	...
	if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
		std::cout << "Cycler::cycleAgeing is initialising the cell\n";
	...
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in a subfunction of Cycler::cycleAgeing when getting the cell to Vma, error " << e << ". Throwing it on.\n";
		throw e;
	}
	...
	{
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::cycleAgeing is doing an initial check-up.\n";
		cap = checkUp(proc, 0, timetot, Ahtot, Whtot);
	}
	catch (int e)
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in the initial check-up Cycler::cycleAgeing " << e << ". Throwing it on.\n";
		throw e;
	}
	...
	{ // loop for the specified number of cycles
		try
		{
			...
			// discharge
			if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
				std::cout << "Cycler::cycleAgeing is discharging the cell in cycle number " << i << ".\n";
			if (CVdis)
			...
			// charge
			if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
				std::cout << "Cycler::cycleAgeing is charging the cell in cycle number " << i << ".\n";
			if (CVcha)
				CC_V_CV_I(Ccha, Vma, Ccutcha, dt, blockDegradation, &ahi, &whi, &ti); // CC and CV charge
			else
				CC_V(-Ccha * capnom, dt, blockDegradation, Vma, &ahi, &whi, &ti); // CC charge (must have current as input, not C rate)
			...
			if (std::fmod(i + 1, nrCap) == 0)
			{ // remainder is 0 -> it is a multiple of nrCap
				if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
					std::cout << "Cycler::cycleAgeing is doing a check-up in cycle number " << i << ".\n";
				...
				if (cap < capnom / 2.0)
				{ // end simulation if the cell has only 50% of capacity left
					std::cout << "Cycler::cycleAgeing has finished cycling regime " << ID << " early because the cell has already lost 50% of its capacity.";
		...
		catch (int e)
		{
			...
			if constexpr (settings::verbose >= printLevel::printCrit)
			{
				std::cout << "Error in Cycler::cycleAgeing while cycling the cell according to  cycling regime " << ID << ". Error encountered is " << e << ". Stop cycling now.";
	...
	if (final)
	{
		...
		{
			if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
				std::cout << "Cycler::cycleAgeing is doing a final check-up.\n";
			checkUp(proc, nrCycles, timetot, Ahtot, Whtot);
		}
		catch (int e)
		{
			if constexpr (settings::verbose >= printLevel::printCrit)
				std::cout << "Error in a in the final check-up of Cycler::cycleAgeing " << e << ". Throwing it on.\n";
			throw e;
		}
	}
...
void Cycler::calendarAgeing(double dt, double V, double Ti, int Time, int timeCheck, int mode, struct checkUpProcedure proc)
{
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::calendarAgeing starting\n";
	...
	if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
		std::cout << "Cycler::calendarAgeing is initialising the cell\n";
	...
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in a subfunction of Cycler::CalendarAgeing when getting the cell to the resting voltage "
					  << e << ". Throwing it on.\n";
		throw e;
	}
	...
	{
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::calendarAgeing is doing an initial check-up.\n";
		cap = checkUp(proc, 0, timetot, Ahtot, Whtot);
	}
	catch (int e)
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in a subfunction of Cycler::CalendarAgeing in the initial check up " << e << ". Throwing it on.\n";
		throw e;
	}
	...
	for (int i = 0; i < nrdt; i++)
	{
		try
		{
			...
			{
				if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
					std::cout << "Cycler::calendarAgeing is resting the cell in period " << i << ".\n";
				CC_t(0, dt, blockDegradation, trest, &ahi, &whi, &ti); // rest (= CC charge at 0A)

				if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
					std::cout << "Cycler::calendarAgeing is recharging the cell in period " << i << ".\n";
			...
			else if (mode == 1)
			{
				...
				{									// a loop for every day
					...
					if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
						std::cout << "Cycler::calendarAgeing is recharging the cell in day " << j << " of period " << i << ".\n";
					CV_I(V, dt, blockDegradation, Ccut, &ahi2, &whi2, &ti2); // recharge to the specified voltage
					timetot += (ti + ti2) / 3600.0;							 // number of hours we have rested additionally
				}
			}
			...
			{
				if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
					std::cout << "Cycler::calendarAgeing is floating the cell in period " << i << ".\n";
			...
			// do a check-up
			if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
				std::cout << "Cycler::calendarAgeing is doing a check-up in period " << i << ".\n";
			cap = checkUp(proc, 0, timetot, Ahtot, Whtot);

			// End the experiment if the cell capacity has decreased too much
			if (cap < capnom / 2.0)
			{ // end simulation if the cell has only 50% of capacity left
				std::cout << "Cycler::CalendarAgeing has finished calendar regime " << ID << " early do to too little remaining capacity.";
		...
		{
			std::cout << "Error in Cycler::CalendarAgeing while resting the cell according to calendar regime " << ID << ". Error encountered is " << e << ". Stop resting now.";
...
void Cycler::profileAgeing(std::string nameI, int limit, double Vma, double Vmi, double Ti, int nrProfiles, int nrCap, struct checkUpProcedure proc, size_t length)
{
	...
	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::profileAgeing starting.\n";

	// Check the input parameters
	bool vmax = Vma > c.getVmax(); // check if the maximum voltage is below the cell maximum voltage
	if (vmax)
		std::cerr << "Error in Cycler::profileAgeing. The maximum voltage " << Vma
	...
	if (vmin)
		std::cerr << "Error in Cycler::profileAgeing. The minimum voltage " << Vmi
	...
	if (Temin)
		std::cerr << "Error in Cycler::profileAgeing. The temperature " << Ti << "K is too low. The minimum value is 273.\n";
	bool Temax = Ti > settings::Tmax_K; // check the temperature is below 60 degrees, TMIN is defined in State.hpp
	if (Temax)
		std::cerr << "Error in Cycler::profileAgeing. The temperature " << Ti << " is too high. The maximum value is (273+60).\n";
	...
	if (cycles)
		std::cerr << "Error in Cycler::profileAgeing. The number of cycles between two check ups " << nrCap
	...
	{
		std::cout << "error in Cycler::profileAgeing when reading the file with the current profile called "
	...
	if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
		std::cout << "Cycler::profileAgeing is initialising the cell.\n";
	...
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in Cycler::profileAgeing when bringing the cell to the initial voltage " << e << ". Throwing it on.\n";
		throw e;
	}
	...
	{
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::profileAgeing is doing an initial check-up.\n";
		checkUp(proc, 0, timetot, Ahtot, Whtot);
	}
	catch (int e)
	{
		if constexpr (settings::verbose >= printLevel::printCrit)
			std::cout << "Error in a subfunction of Cycler::profileAgeing in the initial check-up " << e << ". Throwing it on.\n";
		throw e;
	}
	...
	while (nreptot < nrProfiles)
	{ // loop to follow the profile the set number of times

		// *********************************************************** 3A  follow the profile until you hit a voltage limit ***********************************************************************
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::profileAgeing has applied the profile " << nreptot
		...
		catch (int e)
		{
			if constexpr (settings::verbose >= printLevel::printCrit)
			{
				std::cout << "Error in Cycler::profileAgeing when cycling the cell with the current profile according to profile regime " << ID << ". Error encountered is " << e << ". Stop cycling now.";
		...
		if (nrep <= 1)
		{
			...
			std::cerr << "ERROR in Cycler::profileAgeing: the profile " << ID
		...
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::profileAgeing has applied the profile " << nreptot << " times and is going to re(dis)charge the cell.\n";
		try
		{
		...
		catch (int e)
		{
			if constexpr (settings::verbose >= printLevel::printCrit)
			{
				std::cout << "Error in Cycler::profileAgeing when re(dis)charging the cell according to profile regime " << ID << ". Error encountered is " << e << ". Stop cycling now."
		...
		{
			if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
				std::cout << "Cycler::profileAgeing has applied the profile " << nreptot << " times and is going to do a check-up.\n";
			try
			{
			...
			catch (int e)
			{
				if constexpr (settings::verbose >= printLevel::printCrit)
				{
					std::cout << "Error in Cycler::profileAgeing when doing a check-up according to profile regime " << ID << ". Error encountered is " << e << ". Stop cycling now."
							  << " We have done " << nreptot << " repetitions instead of " << nrProfiles << " and the capacity last measured is " << capi << ".\n";
				}
			...
			// End the experiment if the cell capacity has decreased too much
			if (capi < c.getNominalCap() / 2.0)
			{ // end simulation if the cell has only 50% of capacity left
				std::cout << "Cycler::ProfileAgeing has finished cycling regime " << ID << " early do to too little remaining capacity."
	...
	{
		if constexpr (settings::verbose >= printLevel::printCyclerHighLevel)
			std::cout << "Cycler::profileAgeing has applied the profile " << nreptot << " times and is going to do a final check-up.\n";
		try
		{
		...
		{
			if constexpr (settings::verbose >= printLevel::printCrit)
				std::cout << "Error in a subfunction of Cycler::profileAgeing in the final check-up " << e << ". Throwing it on.\n";
			throw e;
		}
	}

	if constexpr (settings::verbose >= printLevel::printCyclerFunctions)
		std::cout << "Cycler::profileAgeing terminating.\n";
}
