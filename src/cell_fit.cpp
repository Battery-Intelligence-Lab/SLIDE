...
Cell_Fit::Cell_Fit(const slide::Model &MM, int verbosei)
	: Cell(settings::path::Kokam::namepos, 49, settings::path::Kokam::nameneg, 63, settings::path::Kokam::nameentropicC, 11, settings::path::Kokam::nameentropicCell, 11)
{
	...
	// Stress parameters
	sparam = settings::def_param::StressParam_Fit;
...
void Cell_Fit::setOCVcurve(std::string namepos, std::string nameneg)
{
	/*
	...
	try
	{
		loadCSV_2col(PathVar::data + nameneg, OCV_curves.OCV_neg_x, OCV_curves.OCV_neg_y); // the OCV curve of the anode, the first column gives the lithium fractions (increasing), the 2nd column gives the OCV vs li/li+
		loadCSV_2col(PathVar::data + namepos, OCV_curves.OCV_pos_x, OCV_curves.OCV_pos_y); // the OCV curve of the cathode, the first column gives the lithium fractions (increasing), the 2nd column gives the OCV vs li/li+

		OCV_curves.is_OCV_neg_fixed = check_is_fixed(OCV_curves.OCV_neg_x);
		OCV_curves.is_OCV_pos_fixed = check_is_fixed(OCV_curves.OCV_pos_x);

		OCV_curves.OCV_neg_n = OCV_curves.OCV_neg_x.size();
		OCV_curves.OCV_pos_n = OCV_curves.OCV_pos_x.size();
	}
	catch (int e)
	{
		std::cout << "ERROR in Cell_Fit::setOCVcurve when loading the OCV curves from the CSV files: " << e << ". Throwing it on.\n";
		throw e;
	}
}
...
void Cell_Fit::setInitialConcentration(double cmaxp, double cmaxn, double lifracp, double lifracn)
{
	...
	{
		std::cout << "Error in Cell_Fit::setInitialConcentration when setting the concentration: " << e << ". Throwing it on.\n";
		throw e;
	}
}
...
void Cell_Fit::setCharacterisationParam(double Dp, double Dn, double kpi, double kni, double Rdc)
{
	...
	double r = Rdc * ((s.get_thickp() * s.get_ap() * elec_surf + s.get_thickn() * s.get_an() * elec_surf) / 2);
