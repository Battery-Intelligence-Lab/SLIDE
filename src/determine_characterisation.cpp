			  ...
			  slide::vec_XYdata &Vsim, slide::vec_XYdata &Tsim)
{
	...
	// variables
	slide::State s;								   // initial state of the cell, used to recover after an error
	...
	std::string ID = "CharacterisationFit";		   // identification std::string for the Cycler
	...
	for (int i = 0; i < 2 && !finished; i++)
	{
		...
		for (int j = 0; j < 2 && !finished; j++)
		{
			...
			try
			{
				...
				c1.setStates(s, Iini);					// Does not throw.
...
void CCCV(double Crate, double Ccut, double Tref, double Dp, double Dn, double kp, double kn, double R, const struct OCVparam &ocvfit, const struct slide::Model &M,
		  slide::vec_XYdata &Vsim, slide::vec_XYdata &Tsim)
{
	/*
	 * Function which simulates a full CC CV (dis)charge with the given rate parameters at reference temperature.
						 ...
						 double *err, std::array<double, 5> &par, std::array<int, 5> &parindex)
{
	...
	cell_init.setVlimits(ocvfit.Vmax, ocvfit.Vmin);
	...
	std::string ID = "CharacterisationFit";				  // identification std::string for the Cycler
	int timeCycleData = -1;								  // time interval at which cycling data has to be recorded [s]
	Cycler cycler(cell_init, ID, verbose, timeCycleData); // Make the Cycler
...
void estimateCharacterisation()
{
	...
	const int nCCCV = std::size(names); // number of data sets for CCCV (dis)charges
	...
	std::vector<slide::vec_XYdata> Vdata_all(nCCCV);
	...
	{ // loop to read the voltage profile of each cycle
		...
		loadCSV_2col(PathVar::data + names[i], Vdata_all[i].x, Vdata_all[i].y); // read the csv file with the voltage profile
		...
		if (!val)
		{
			std::cerr << "ERROR in determineCharacterisation::fitDiffusionAndRate. Input file " << names[i] << " has the wrong format. throwing an error.\n";
			throw 10000;
		}
	}
	...
	slide::Model M; // structure with the matrices for the spatial discretisation for the solid diffusion PDE #CHECK possible duplication somewhere of below code.
	...
	slide::vec_XYdata Vsim, Tsim; // arrays to store the simulation results #CHECK
