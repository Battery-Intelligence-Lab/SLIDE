...
void Cell_SPM::getConcentration(int nin, double cp[], double cn[])
{
  ...
  if (nin != CELL_NCH + 2) { // nch is the number of inner nodes, +2 for the node at the centre and surface
  ...
  double cpt, cnt;
  for (int i = 0; i < CELL_NCH + 1; i++) { // loop to calculate at each surface + inner node
    cpt = 0.0;
    cnt = 0.0;
    for (int j = 0; j < CELL_NCH; j++) {
...
void Cell_SPM::getDaiStress(int nin, double *sigma_p, double *sigma_n, double sigma_r_p[], double sigma_r_n[], double sigma_t_p[], double sigma_t_n[], double sigma_h_p[], double sigma_h_n[])
{
  /*
  ...
  for (int i = 0; i < 2 * CELL_NCH + 3; i++) {          // loop for each row (one row = one node)
    ...
    for (int j = 0; j < 2 * CELL_NCH + 3; j++) {        // loop through the columns to calculate the sum
  ...
  for (int i = 0; i < CELL_NCH + 2; i++) { // loop for the positive nodes
  ...
  for (int i = 0; i < CELL_NCH + 2; i++) {                // loop for all nodes
...
double Cell_SPM::setSoC(double SoCnew, bool checkV, bool print)
{
  ...
  if (checkV) {
    ...
    if (err == -2 || err == 2) { // safety limits VMIN and VMAX
      SoC = SoCold;
      if (verb)
    ...
    if (err == -1 || err == 1) { // valid limits Vmin and Vmax
      if (verb)
...
double Cell_SPM::setI(double Inew, bool checkV, bool print)
{
  ...
  if (checkV) {
    ...
    // check if the voltage is valid
    if ((err == -2 && I > 0) || (err == 2 && I < 0)) { // outside safety limits (< VMIN and discharge || > VMAX and charge)
      I = Iold;
      if (verb)
    ...
    if ((err == -1 && I > 0) || (err == 1 && I < 0)) { // outside valid limits for the cell
      if (verb)
...
void Cell_SPM::setC(double fp0, double fn0)
{
  ...
  for (int i = 0; i < CELL_NCH; i++) { // loop to calculate the row of V * u corresponding to the uniform concentration
  ...
  double zp[CELL_NCH], zn[CELL_NCH];
  for (int i = 0; i < CELL_NCH; i++) { // set all values to 0
    zp[i] = 0;
    zn[i] = 0;
  }
...
void Cell_SPM::dState_diffusion(bool print, double dzp[], double dzn[], double &dSoC)
{
  ...
  double ctep, cten;
  for (int j = 0; j < CELL_NCH; j++) { // loop for each row of the matrix-vector product A * z
