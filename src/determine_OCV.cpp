...
auto discharge_noexcept(const slide::vec_XYdata &OCVp, const slide::vec_XYdata &OCVn, double cap, const double AMp, const double AMn, const double cmaxp,
						const double cmaxn, double sp, double sn, double Vend, slide::vec_XYdata &OCV, slide::vec_XYdata &OCVanode,
						slide::vec_XYdata &OCVcathode, double fp[], double fn[])
{
	/*
	...
	// auto Ah_fixed = slide::range_fix(Ah, cap, I * dt / 3600);
	// auto sp_fixed = slide::range_fix(sp, 1.0, I * dt / (n * F) / AMp / cmaxp);
	// auto sn_fixed = slide::range_fix(sn, 0.0, -I * dt / (n * F) / AMn / cmaxn);

	// auto Ah_n = Ah_fixed.size();
	// auto sp_n = sp_fixed.size();
	// auto sn_n = sn_fixed.size();

	// auto s_max = std::min(sp_n, sn_n);
	// auto Vstop = 0;
	...
	// 		std::tie(ocvpi, flag_ocvpi) = linInt_noexcept(bound, OCVp.x, OCVp.y, OCVp.size(), sp_fixed[i]);
	// 		std::tie(ocvni, flag_ocvni) = linInt_noexcept(bound, OCVn.x, OCVn.y, OCVn.size(), sn_fixed[i]);

	// 		auto V = ocvpi - ocvni;

	// 		std::cout << "V = " << V << " ocvpi = " << ocvpi << " ocvni = " << ocvni << " sp = " << sp_fixed[i] << " sn = " << sn_fixed[i] << "\n";
	// 		if (V <= Vend)
	// 		{
	// 			s_max = i + 1;
	// 			Vstop = V;
...
auto discharge_noexcept(const slide::vec_XYdata &OCVp, const slide::vec_XYdata &OCVn, double cap, const double AMp, const double AMn, const double cmaxp,
						const double cmaxn, double sp, double sn, double Vend, slide::vec_XYdata &OCV, double fp[], double fn[])
{
	/*
	 * Function to simulate a CC discharge at a very low current, so we can ignore resistances and diffusion.
	 *
	...
	// If we store a data point for every time step, the output arrays will be very long.
	// Therefore, store only one value every 'nstore' time steps
	size_t nstore = 100; // store one data point every 100 time steps
...
void discharge(const slide::vec_XYdata &OCVp, const slide::vec_XYdata &OCVn, double cap, const double AMp, const double AMn, const double cmaxp,
			   const double cmaxn, double sp, double sn, double Vend, slide::vec_XYdata &OCV, slide::vec_XYdata &OCVanode,
			   slide::vec_XYdata &OCVcathode, double fp[], double fn[])
{
	/*
	...
	const auto flag = discharge_noexcept(OCVp, OCVn, cap, AMp, AMn, cmaxp, cmaxn, sp, sn, Vend, OCV, OCVanode,
										 OCVcathode, fp, fn);
				  ...
				  slide::vec_XYdata &OCVp, slide::vec_XYdata &OCVn, slide::vec_XYdata &OCVcell)
{
	...
	try
	{
		loadCSV_2col(PathVar::data + namepos, OCVp.x, OCVp.y);
		loadCSV_2col(PathVar::data + nameneg, OCVn.x, OCVn.y);
		loadCSV_2col(PathVar::data + namecell, OCVcell.x, OCVcell.y);
	}
	catch (int e)
	{
	...
	bool valp = validOCV(true, OCVp); // check the cathode OCV curve. The first column must go from 0-1
	if (!valp)
	{
		std::cerr << "ERROR in determineOCV::readOCVinput, the file with the cathode OCV curve " << namepos << " has an illegal format.\n";
		throw 10000;
	}
	bool valn = validOCV(true, OCVn); // check the anode OCV curve. The first column must go from 0-1
	if (!valn)
	{
		std::cerr << "ERROR in determineOCV::readOCVinput, the file with the anode OCV curve " << nameneg << " has an illegal format.\n";
		throw 10000;
	}
	bool valcell = validOCV(false, OCVcell); // check the cell OCV curve. The first column must start at 0 but can end anywhere
	if (!valcell)
	{
...
double calculateError(bool bound, slide::vec_XYdata &OCVcell, slide::vec_XYdata &OCVsim)
{
	...
	for (size_t i = 0; i < OCVcell.size(); i++)
	{
		...
		auto [Vsimi, status] = linInt_noexcept(bound, OCVsim.x, OCVsim.y, OCVsim.size(), OCVcell.x[i]);
						...
						slide::fixed_data<double> sn_space, std::string namepos, std::string nameneg, std::string namecell, double cmaxp, double cmaxn)
{
	...
	slide::vec_XYdata OCVp(100), OCVn(100), OCVcell(100); // Temp vectors
	...
	for (int h = 0; h < hmax; h++)
	{
		...
		auto task_indv = [&](int i)
		{
			...
			fitAMnAndStartingPoints(h, i, AMp1, AMn_space, sp_space, sn_space, cmaxp, cmaxn,
									&err_arr[i], par_arr[i], OCVp, OCVn, OCVcell);
		};
...
void estimateOCVparameters()
{
	...
	// Read the OCV curves
	slide::vec_XYdata OCVp(np), OCVn(nn), OCVcell(ncell);
	try
	{
		readOCVinput(namepos, nameneg, namecell, OCVp, OCVn, OCVcell);
	}
	...
	const double step = 0.1; // take steps of 10% of the guessed active material
	const double AMmax = 5;	 // the maximum amount of active material is 5 times the guessed amount
	const double AMmin = 0;	 // the minimum amount of active material is 0, actually should not be zero since it is a divisor.

	// // Define the search space for the initial lithium fractions at each electrode
	const double df = 0.1;	 // take steps of 10% of the lithium fraction
	const double fmin = 0.0; // the minimum lithium fraction is 0
	const double fmax = 1.0; // the maximum lithium fraction is 1
	...
	slide::vec_XYdata OCVsim, OCVnsim, OCVpsim;						 // array for the simulated OCV curve and voltage of each electrode
