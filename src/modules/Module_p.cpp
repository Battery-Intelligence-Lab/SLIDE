...
namespace slide
{
	...
	Module_p::Module_p(std::string IDi, double Ti, bool print, bool pari,
					   int Ncells, int coolControl, int cooltype)
		: Module_p()
	{
		/*
	...
	Status Module_p::redistributeCurrent(bool checkV, bool print)
	{
		...
		while (!reached) // #CHECK if this algorithm is efficient.
		{
			...
			if (!reached)
			{
				...
				// update voltages
				if (noRc)
				{ // no contact resistance, so only for cells which change current, does the V change
					try
					{
	...
	bool Module_p::validSUs(moduleSUs_span_t c, bool print)
	{
		...
		auto Vi = [N = getNSUs(), c, print, this](size_t i)
		{
			...
			for (size_t j = 0; j <= i; j++)
			{ // account for the contact resistances
				double Ij = 0;
	...
	void Module_p::timeStep_CC(double dt, bool addData, int nstep)
	{
		/*
		...
		{ SUs[i]->timeStep_CC(dt, addData, nstep); };
