...
| P9-B30 | High | ThreadPool failure determinism | `first_status_` was won by scheduling-order CAS, so two failing indices could return different statuses across runs despite the deterministic contract. A controlled three-task test forced index 1 to publish before index 0 and failed its expected lowest-index status. | Record the lowest failing index and matching status under the existing mutex after every task; all tasks still finish. Add exception/reuse and overlapping-submission coverage. | Pre-fix focused test 1 failure; Debug/Release ThreadPool gates pass. | FIXED |
| P9-B31 | Medium | ThreadPool public/build boundary | A const lvalue callback failed to compile because its pointer could not convert to `void *`; a null function pointer could be invoked; terminal cursor fetches could wrap near `SIZE_MAX`; and the core target omitted explicit thread linkage. | Use a typed stack context that preserves callback cv-qualification, reject null callbacks, bound `count` against terminal worker fetches, and link public `Threads::Threads` through the shared core-target helper. | Standalone const-callback compile probe was red before and green after; Debug/Release null/empty/reuse gates pass. | FIXED |
| P9-B32 | Critical | production batch parallelism absent | P8-G4's pool had no production caller: both Thevenin archetype linearization and `PackStepper` advance remained serial. A bounded overlap probe timed out with `max_active=1` and failed one assertion. | D-30 adds a movable `BatchExecutor` owned by `PackSolver`, shared by Thevenin linearization and PackStepper stepping. Only independent batch kernels run concurrently; serial gathers/scatters/reductions and substep barriers preserve order. | Overlap probe reaches at least 2; 1/2/7-worker pack solutions are bit-identical; Debug/Release PackSolver 702, PackStepper 175, and multi-batch zero-allocation 14 pass. | FIXED |
| P9-B33 | High | legacy parallel facade | If `hardware_concurrency()` returned zero, `slide::run` created zero workers and silently executed no tasks. An exception escaping a worker called `std::terminate`; the first fix still skipped later tasks with one selected worker, and partial thread-creation failure could destroy joinable threads. | Normalize unknown hardware to one, cap workers to task count, catch per-task exceptions while completing all indices for both inline and threaded execution, deterministically rethrow the lowest-index exception after joining, join created workers on construction failure, and use an unsigned stride that cannot overflow at `INT_MAX`. The v3 facade remains a documented strangler exception to persistent pooling. | Helper covers zero hardware and task caps; one- and two-worker exception cases each visit all six indices exactly once and rethrow `"lowest"` in both builds. | FIXED |
| P9-B34 | High | fixed-order sum under fast-math | The plain source-order loop was compiled with Release reassociation enabled, so its self-referential test could not prove the reduction order. Removing strict-FP protection makes the 3,072-value cancellation oracle return bit pattern `0x4014000000000000` (5.0) instead of +0.0. | Disable reassociation/contraction locally and force each scalar accumulation through a volatile stage; compare against independent exact cancellation results rather than another call to the same helper. | Mutation-red optimized oracle; clean Debug/Release ThreadPool gate passes. | FIXED |
| P9-B35 | Medium | Clang/CMake thread portability | A fresh Linux Clang 18 core-only configure failed every `FindThreads` probe: CMake 3.31 tried to launch `CMAKE_CXX_COMPILER_CLANG_SCAN_DEPS-NOTFOUND` while scanning its own `try_compile`, even though SLIDE contains no C++ modules. | Set `CMAKE_CXX_SCAN_FOR_MODULES=OFF` immediately after `project()`, so project targets and configure-time probes share the truthful no-modules policy. | Fresh WSL Ubuntu/Clang 18 finds libc pthreads, builds all core objects, links the public consumer, and passes core-only smoke 1/1. | FIXED |
| P9-B36 | Critical | parallel batch aliasing | Two archetype entries could bind the same Thevenin object or `SpmBatch*`; the new pool would then race pipeline scratch/cache or advance one arena through two steppers. Both pre-fix configure regressions returned `Success` and failed one assertion. | Expose opaque `TheveninBatchView` identity and reject duplicates in `PackTheveninSystem`; independently reject repeated `SpmBatch*` in `PackStepper` before executor creation. | Both alias regressions reject cold in Debug/Release. Real two-archetype 1-vs-2-worker runs are byte-identical across both arenas, solution, diagnostics, cell heat, and boundary heat. | FIXED |
