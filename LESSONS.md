...
- A final converged solution can hide an operation-order change that occurred one iteration earlier. For bit-preserving algebra refactors, record the callback/iterate frame at the changed seam, construct a non-dyadic input that separates `/R` from `*(1/R)`, and mutate every duplicated site separately; keep an analytic conservation oracle beside the compiler-specific fingerprint.
- A structural owner gate must pin the semantic body and same-typed argument order, not merely count a helper name. CMake variadic arguments split semicolon-rich C++ bodies, and comment-stripped input cannot validate documentation adjacency; use fixed-arity exact blocks for code and a separate raw-comment view, then mutation-test both.
