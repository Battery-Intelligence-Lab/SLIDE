function get_model_vk_slide_impl(Nch)
...
for i=1:Ncheb
    ...
    for j = 1:Ncheb
        ...
        D_vk(i,j) = (C_vk*D_vk(i,i) - D_vk(i,j))/DX_vk;
...
DN2 = DM2(:,1:N) - fliplr(DM2(:,N+2:M+1));
DN1 = DM1(1,1:N) - fliplr(DM1(1,N+2:M+1));
...
Ln = Dn;
Vn = Vn;
An = Dn;       % diagonal matrix with eigenvalues
Bn = Vn\B1;    % V^(-1)*B
Cn = C1*Vn;    % row1 = surface (node N+1), last row = close to centre (node 2)
Dn = D1;

[Vp,Dp] = eig(A3);
Lp = Dp;
Vp = Vp;
Ap = Dp;       % diagonal matrix with eigenvalues
Bp = Vp\B3;    % V^(-1)*B
Cp = C3*Vp;    % row1 = surface (node N+1), last row = close to centre (node 2)
Dp = D3;

Cc = (DM1(1,1:N,1) + fliplr(DM1(1,N+2:Ncheb,1)))';  % matrix to get the concentration at the centre node
Vn = inv(Vn);
Vp = inv(Vp);
...
Q = cumsummat(M+1);
Q(1,:) = 0;        % due to numerical errors it is not exactly 0, but it is the integral from the centre to the centre, so it should be 0
