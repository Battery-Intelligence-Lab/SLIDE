...
xr = xm(1:N+1); %  Half domain [ 0,R]->[ 0,1]
...
%% Transformation to eigenvector-basis z (instead of u)
    ...
    model.Bn = Vn\model.B1;    % V^(-1)*B
    model.Cn = model.C1*Vn;    % row1 = surface (node N+1), last row = close to centre (node 2)
    model.Dn = model.D1;
    ...
    model.Bp = Vp\model.B3;    % V^(-1)*B
    model.Cp = model.C3*Vp;    % row1 = surface (node N+1), last row = close to centre (node 2)
    model.Dp = model.D3;
